...
-   Difference: *The quantity that the MetaScore (aggregate score of critics' ratings) differs from the IMDB score (score given by audience/fans on IMDB, scaled to match MetaScore)*
...
The final model we chose is our full model, fit with gross income and runtime mean-centered, decade, censorship certificate, number of votes and the interaction between gross and certificate. 

```{r}
tidy(imdb_full_log) %>% 
  kable(digits = 3)
```
To address our research question, we fit a logistic regression model predicting the odds that a movie’s IMDb audience score exceeds its critic MetaScore. The final model included centered and scaled versions of gross revenue, runtime, and number of IMDB vores, as well as release decade, certificate rating, and an interaction term between gross revenue and certificate. A drop-in-deviance test confirmed that including certificate and its interaction with gross significantly improved model fit ($\chi^2$ = 19.648, df = 8, p = 0.012). The full model also had a lower AIC than the reduced model, indicating a more favorable balance between model complexity and fit.
