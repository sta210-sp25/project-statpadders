...
```
  ...
  In terms of the `difference` between the two scores, it seems that the values are almost normally distributed, with a slight left skew. This suggests that it is nearly equally common for a IMDB Score to be either higher or lower than the MetaScore score, though slightly more often lower. Additionally, when plotting our numerical predictors, there appear to be no linear relationships between each predictor and our response. *See appendix for full EDA*
