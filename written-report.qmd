...
We began our study with comprehensive exploratory data analysis to assess variable relationships and potential modeling challenges. A correlation matrix identified strong correlation (0.62) between the IMDB score and number of votes a film received, leading us to avoid including both in the same model to prevent multicollinearity. Additionally, initial visualizations suggested a potential interaction effect between a film’s censorship certificate and its gross revenue. These findings informed the structure of our modeling approach. When plotting our numerical predictors, there appear to be no linear relationships between each predictor and our response. (*See Appendix for full EDA.*)
...
Our goal is to capture as many truly divergent films as possible, even if it means accepting a slightly higher false positive rate. Lowering the classification threshold to 0.3 significantly improved the model’s sensitivity, allowing it to identify a much greater proportion of the films where audience scores diverged meaningfully from critic scores. This came at the expected cost of increasing false positives, resulting in a moderate drop in specificity and overall precision.

To assess the model’s overall discriminative ability independently of any fixed threshold, we generated an ROC curve and calculated the Area Under the Curve (AUC), which came out to approximately 0.67. Since an AUC of 0.5 would indicate random guessing, a score of 0.67 suggests that while the model performs better than chance at ranking films by their likelihood of divergence, its ranking ability remains moderate. This reflects the fact that while our predictors do capture some meaningful signal, they are limited in their ability to fully distinguish divergent films.
The model's performance illustrates the constraints of relying solely on structural attributes such as gross revenue, runtime, decade, and number of votes to predict audience–critic divergence. These features are broad in scope. Regression models do not have the ability to account for more subjective, content-based factors that likely play a central role in shaping divergent reception. Additionally, the relationship between commercial performance and critical reception has evolved over time. Shifts in genre popularity, the rise of blockbuster franchises, changing cultural norms, and the influence of streaming platforms are all confounding variables that have likely contributed to a more complex and less predictable dynamic between critics and general audiences.
As such, while our model provides useful insight into patterns of rating divergence, its limitations underscore the need for more nuanced data. Future research could benefit from incorporating content-rich variables such as genre, award recognition, sentiment analysis of reviews, or audience demographics to develop a more comprehensive understanding of the factors that drive divergence between audience and critic evaluations.
